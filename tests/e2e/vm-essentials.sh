...
assert_cmd "chrony package or binary" bash -c 'command -v chronyd || command -v chronyc || { command -v dpkg >/dev/null 2>&1 && dpkg -s chrony >/dev/null 2>&1; } || { command -v rpm >/dev/null 2>&1 && rpm -q chrony >/dev/null 2>&1; }'
