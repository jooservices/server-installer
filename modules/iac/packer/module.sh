#!/usr/bin/env bash
# Install Packer CLI (HashiCorp). Install-only.
...
module_plan() {
  if module_check; then log_plan "Packer already installed"; else log_plan "Will install Packer from HashiCorp repo"; fi
}
...
module_verify() { command -v packer >/dev/null 2>&1; }
